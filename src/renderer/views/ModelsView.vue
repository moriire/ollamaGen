...
onMounted(()=>{
  getModels()
    ...
    return {
      formatdate, getModels, models, disable, pullModel, chosen_model, progress,
      params
      }
  }    
}
...
<template>
  ...
  <div class="form-group mb-3 input-group-text">
      ...
      <input placeholder="OpenAI API Key" type="text" class="form-control" v-model="chosen_model">
...
<div class="col-lg-10 col-md-10 col-sm-10 col-xs-12 mx-3 my-2">
  ...
  <div class="table-responsive text-light">
    <table class="table text-light table-bordered table-hover table-striped table-light">
      <tr v-for="model in models" :key="model.digest">
        <td>
          <input class="form-check-input" type="radio" v-model="params.selectedModel" :value="model.name" @change ="params.selectModel">
      <span class="bg-black text-light">{{ model.name }} </span>
        
        </td>
        <td>
          {{ model.details.parameter_size }}<br>
